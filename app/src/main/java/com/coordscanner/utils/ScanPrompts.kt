...
internal object ScanPrompts {
    ...
    fun sk42Columns(nameFr: NormRect, xFr: NormRect, yFr: NormRect): String = """
        ...
        • Копируй имя ДОСЛОВНО как в таблице — со всеми символами, тире, кавычками,
          точками, скобками, пробелами. Не сокращай, не переводи. Имя может быть
          длинным (до 256 символов) — переноси целиком.
    ...
    fun wgsColumns(nameFr: NormRect, latFr: NormRect, lonFr: NormRect): String = """
        ...
        • Если имя пустое или нечитаемо — "Точка".
        • Копируй имя ДОСЛОВНО как в таблице — со всеми символами, тире, кавычками,
          точками, скобками, пробелами. Не сокращай, не переводи, не подставляй "Точка"
          если в строке есть реальное название.
        • Имя может быть длинным (до 256 символов) — переноси целиком.
        • Цифры распознавай аккуратно (0/O, 1/I/l, 5/S, 8/B).
        ...
        Пример валидного ответа (без markdown, без обёртки):
        [
          {"name":"г. Долгопрудный – ЗАО «Универсал Нефть»","lat":55.969675,"lon":37.467136},
          {"name":"– позиция ЗРПК «Панцирь С1» – южн. окраина н.п. Прилепы","lat":50.925543,"lon":36.609114}
        ]
